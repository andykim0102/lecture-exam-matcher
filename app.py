...
# ==========================================
# 4. [NEW] Parsing & Twin Question Logic
# ==========================================
def parse_jokbo_with_llm(raw_text):
    """엉망인 텍스트를 구조화된 JSON으로 변환"""
    prompt = f"""
    You are an expert exam parser.
    Analyze the following unstructured or messy exam question text.
    Extract the 'question', 'choices' (if multiple choice, list them), 'answer' (correct option or text), and 'explanation' (if present).
    
    Rules:
    - Respond ONLY with a valid JSON object.
    - Keys: "question", "choices", "answer", "explanation".
    - If "choices" are not present (short answer), use an empty list [].
    - If "answer" is not explicitly found, infer it or leave as "Unknown".
    - "explanation" can be empty if not found.
...
def generate_twin_problem_from_parsed(parsed_data):
    """구조화된 데이터를 기반으로 쌍둥이 문제 생성"""
    prompt = f"""
    Based on the following structured exam question, create a "Twin Question" (a variation testing the same concept).
...
def build_overview_prompt(first_page_text, subject):
    return f"""
    너는 의대 수석 조교다. 지금 학생이 '{subject}' 강의록의 첫 페이지를 보고 있다.
    이 강의록 전체를 공부할 때 어떤 마음가짐과 전략을 가져야 하는지, 족보 패턴을 고려하여 조언해라.
...
def build_page_analysis_prompt(lecture_text, related_jokbo, subject):
    ...
    [SECTION: TWIN_Q]
    위 족보 문제 중 가장 중요한 것 하나를 골라 유사한 '쌍둥이 문제(변형 문제)'를 1개 만들어라.
    **Q. 문제 내용...**
    1) 보기 ...
    2) 보기 ...
                                            ...
                                            if st.session_state[parse_key] is None:
                                                if st.button("1단계: 문제 구조화 (파싱)", key=f"btn_parse_{idx}"):
                                                    with st.spinner("텍스트 구조 분석 중..."):
                                                        parsed = parse_jokbo_with_llm(raw_q_text)
                                                        st.session_state[parse_key] = parsed
                                                        st.rerun()
                                            
                                            parsed_res = st.session_state[parse_key]
